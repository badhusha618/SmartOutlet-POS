# SmartOutlet POS - Simple Backend Startup Script for Java 17
...
# Function to start a service using Spring Boot
function Start-SpringBootService {
    ...
    if (Test-Path $serviceDir) {
        ...
        try {
            ...
            $process = Start-Process -FilePath "java" -ArgumentList "-jar", "target\$ServiceName-1.0.0.jar", "--server.port=$Port" -WindowStyle Hidden -PassThru
...
# Try to build services using Java directly
Write-Host "Attempting to build services..." -ForegroundColor Yellow

# Check if there are pre-built JAR files
$authJar = "backend\auth-service\target\auth-service-1.0.0.jar"
$outletJar = "backend\outlet-service\target\outlet-service-1.0.0.jar"
$productJar = "backend\product-service\target\product-service-1.0.0.jar"

if (Test-Path $authJar) {
    Write-Host "✅ Pre-built JAR files found" -ForegroundColor Green
} else {
    Write-Host "⚠️  No pre-built JAR files found. Services may not start properly." -ForegroundColor Yellow
    Write-Host "   You may need to build the services first using Maven." -ForegroundColor Yellow
}
...
Start-SpringBootService "auth-service" "auth-service" 8081
Start-Sleep -Seconds 5

# Start other services
Start-SpringBootService "outlet-service" "outlet-service" 8082
Start-SpringBootService "product-service" "product-service" 8083
Start-SpringBootService "inventory-service" "inventory-service" 8086
Start-SpringBootService "pos-service" "pos-service" 8084
Start-SpringBootService "expense-service" "expense-service" 8085
Start-Sleep -Seconds 5

# Start API gateway last
Start-SpringBootService "api-gateway" "api-gateway" 8080
...
Write-Host ""
Write-Host "Note: If services don't start, you may need to build them first using Maven." -ForegroundColor Yellow 